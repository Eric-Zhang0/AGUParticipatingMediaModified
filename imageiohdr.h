...
{
  bool operator()(FILE *in_F, const T *in_Data, int32_t in_NumPixels) { return true; }
};

template <typename T>
struct hdrWritePixels<T, 1>
{
  bool operator()(FILE *in_F, const T *in_Data, int32_t in_NumPixels)
  {
...
inline bool hdrWriteBytesRLE(FILE *in_F, unsigned char *in_Bytes, int32_t in_NumBytes)
{
  ...
  while (cur < in_NumBytes)
  {
    ...
    while ((run_count < MINRUNLENGTH) && (beg_run < in_NumBytes))
    {
      ...
      run_count = 1;
      while ((beg_run + run_count < in_NumBytes) && (run_count < 127) && (in_Bytes[beg_run] == in_Bytes[beg_run + run_count]))
        run_count++;
    }
...
{
  bool operator()(FILE *in_F, const T *in_Pixels, int32_t in_LineWidth, int32_t in_NumLines) { return true; }
};

template <typename T>
struct hdrWritePixelsRLE<T, 1>
{
  bool operator()(FILE *in_F, const T *in_Pixels, int32_t in_LineWidth, int32_t in_NumLines)
  {
    unsigned char rgbe[4];
